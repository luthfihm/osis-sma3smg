...
if ($submit){ 
	...
	if(!eregi("$adminDomain", $ref) && !eregi("$domainAlias", $ref)) {
		echo "<script>alert('ERROR: You can only post to the guestbook from the $adminDomain website.'); location.href='guestbook.php?assign_num=false';</script>"; exit;
	}
	$register_globals = (bool) ini_get('register_gobals');
	if ($register_globals) $ip = getenv(REMOTE_ADDR);
	else $ip = $_SERVER['REMOTE_ADDR'];

	$log2 = fopen("logs/$ip-numVal.db", "r");
	...
	$domain=trim($domain); 
	if(stristr($name, 'admin') != FALSE){ echo "<script>alert('ERROR: That username is banned from gerneral use. Please try again.'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
	if(stristr($name, $adminName) != FALSE){ echo "<script>alert('ERROR: That username is banned from gerneral use. Please try again.'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
        if($name == $namePass) {$name = $adminName; $showIP = 0;}
	if (!$name || $name == "your name"){$name = "Anonymous:";} 
        else{$name .= ":";}
        if($domain == $adminDomain || $domain == $domainAlias) {
                  echo "<script>alert('ERROR: That domain name is banned from general use. Please try again.'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;      
        }
        if($domain == $domainPass) $domain = $domainAlias;
        if (!$message || $message == "your message") {
                  echo "<script>alert('ERROR: You must enter a valid message in order to post. Please try again.'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;
        }
		
		// word filter
		if(stristr($message, 'asu') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'bajingan') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'tai') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'kontol') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'jembut') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'jembud') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'silit') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'manuk') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'lonte') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'kopet') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'fuck') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'ass') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'asshole') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'bitch') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'suck') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'porn') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'babes') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
if(stristr($message, 'blowjob') != FALSE){ echo "<script>alert('ERROR: No DIRTY word, please! -OSIS SMAN 3'); location.href='guestbook.php?assign_num=false';</script>";
                  exit;}
	...
	$name = preg_replace("/</","&lt;",$name);
       ...
       if($showIP != "0"){
       		if($ban_url=="1"){
			function no_url(){ 
				echo "<script>alert('ERROR: Posting web addresses is not allowed. Please try again.'); location.href='guestbook.php?assign_num=false';</script>"; exit; 
			}
	...
	if ($message != "") {
		$df = fopen($dataf, "w+");
		$message = stripslashes($message);
		fwrite($df, "<font face='$fontFace' size='$displaySize'><i style='color:$nameColor'>$name</i></font>\n");
                if($showIP == "1") fwrite($df, "<font face='$fontFace' size='$displaySize'><font style='color:#FFFFFF' size='1'>IP address: <span>$ip</span></font></font><br>");
                $sDate = date('m d y');
                $lDate = date('F j, Y, g:i a');
                if($dateEntries == "1")  {
                       if($shortDate == "1")  {
                              fwrite($df, "<font face='$fontFace' size='$displaySize'>Date: <span style='color:$dateColor'>$sDate;</span></font>\n"); 
                       }
                       if($longDate == "1"){
                              fwrite($df, "<font face='$fontFace' size='$displaySize'>Date: <span style='color:$dateColor'>$lDate</span></font>\n"); 
                       }
                }
                if (!$domain){
                	   fwrite($df, "<font face='$fontFace' size='$displaySize'>Message: <span style='color:$messageColor'>$message</span></font><br>\n");
                } 
                else {
                	fwrite($df, "<font face='$fontFace' size='$displaySize'>Artilce <a href='http://$domain' target=_blank>$domain</a></font>\n");
                	fwrite($df, "<font face='$fontFace' size='$displaySize'>Message: <span style='color:$messageColor'>$message</span></font><br>\n");
                }
	...
	$assign_num="expired"; 
	$l=fopen("logs/$ip-numVal.db", "w+"); fputs($l, "");  fclose($l); 
	$l=fopen("jscript/$ip.js", "w+"); fputs($l, "");  fclose($l);
} 
?>
<?php if ($nr_mouse=="1"){echo "<script src='jscript/nrc.js'></script>";} ?>
<script src='jscript/<?php echo $ip ?>.js'></script>
<form method="POST" style="margin=0" action="guestbook.php?assign_num=false">
