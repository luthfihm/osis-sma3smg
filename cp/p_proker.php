<?
	...
	$view=$op->cs_query("SELECT id FROM tb_proker ORDER BY id DESC");
	...
	$save=$op->save("tb_proker","id,kasie,subsie,nama_proker,title,images,proker","'$noSkr','$_POST[kasie]','$_POST[subsie]','$_POST[nama]','$_POST[title]','$_POST[images]','$cont'");
	if($save){
if ( $_POST['kasie']=="I" ) {
	$kas="1";
	...
	$kas="10";
}else if ( $_POST['kasie']=="OSIS" ) {
	$kas="0";
}
?>
			 	<div><img src="../image/sign_ok_big.png" border="0" align="absmiddle" /> <b>Proses Input Data Subsie Baru Berhasil Dilakukan.</b><br><br>
						<div>Klik <a href="../proker/index.php?kasie=<? echo $kas ?>&&id=<? echo $noSkr ?>" target="_blank">disini</a> untuk melihat halaman Subsie yang baru dimasukan. 
				</div>		
				</div>
			 <?
	}else{
